...
void app_main(void) {
  ...
  char *conn_str = CONFIG_DEVICE_ID;
  esp_mqtt_client_publish(client, "/topic/connect", conn_str, strlen(conn_str),
                          1, 0);

  TimerHandle_t timer1 =
      xTimerCreate("toggler", pdMS_TO_TICKS(CONFIG_BLINK_PERIOD), pdTRUE, 0, timer1_callback);
  if (timer1 != NULL) {
